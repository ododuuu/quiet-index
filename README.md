...
目前版本為 **0.28.0**：互動搜尋可用 `/ 關鍵字` 在目前全部命中文件中逐層縮小，`back` 撤回、`reset` 回到最初結果，並顯示條件鏈與目前／最初總數。0.27.0 起搜尋會顯示總命中數、頁碼與本頁範圍；互動終端可用 `n`／`p` 翻頁，管線或批次可用 `--page`／`--page-size`。`.xml` 原文逐行索引，標籤、屬性和值都可搜尋。既有索引執行一次普通 `index` 即可重試原為 `unsupported` 的 XML，不需要 rebuild。0.28.0 公司 Windows 尚待本版驗收。
