...
目前版本為 **0.29.0**：已登錄子根後，可再 `index` 上層目錄做範圍合併，文件代碼與排除規則會保留；對已包含於上層的子目錄只同步該子樹。0.28.0 起互動搜尋可用 `/ 關鍵字` 在目前全部命中文件中逐層縮小，`back` 撤回、`reset` 回到最初結果。0.27.0 起搜尋會顯示總命中數、頁碼與本頁範圍。`.xml` 原文逐行索引。0.28.0 已由使用者回報驗測成功；0.29.0 公司 Windows 以 `D:\備份` 擴大至 `D:\` 的流程尚待驗收。
