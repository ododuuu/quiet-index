...
最後更新：2026-09-22（0.29.1 人工結果補登；M27／0.30.0 規格完成，待 Grok 實作）

## 目前狀態

- 目前進行中里程碑：M27／0.30.0 原始碼、Big5 與索引觀測。SPEC §38／D046 已規劃，尚未實作；產品 package 仍為 0.29.1。Codex 本次只提交規格，由 Grok 接續實作。
- 本版目標：java／sql／js 原文搜尋，Big5／CP950 解碼及既有文字一次性增量升級，簡潔百分比進度、錯誤彙總、status --issues／--types 與索引容量。class 不支援正文；背景更新管理另列待辦。
...
- 本次處理狀態：indexed 11,534、no_text 561、unsupported 244,403、too_large 0、encrypted 0、error 66；掃描／讀取錯誤 13，同步完整為否，未確認的舊資料被保留。這些是本次處理量，不是全庫累計狀態；不將人工成功擴張為所有格式或全套自動測試通過。
...
- XML、PPTX、PDF 的具體失敗根因尚未取得完整錯誤碼；XML 支援明確編碼宣告，不得直接推定所有 XML error 都是 Big5。下一步由 Grok 依 SPEC §38 實作並補測，交付後再比較新增正文容量、增量耗時與錯誤分布。
- 本次僅文件檢查（git diff --check、範圍與引用核對），未修改程式、未重新跑產品測試或打包。下列 0.29.1 自動測試為原交付歷史結果。
