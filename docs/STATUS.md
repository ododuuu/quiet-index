...
最後更新：2026-09-22（0.28.0 M25 結果內搜尋已實作；公司 Windows 待驗收）

## 目前狀態

- 目前進行中里程碑：M25 結果內搜尋（0.28.0）已依 SPEC §36／D044 實作。互動 `search` 可用 `/ 關鍵字` 在目前完整命中集合追加文件級 AND，`back` 撤回、`reset` 回到最初結果；顯示條件鏈、目前／最初總數與最新條件片段。單頁與零結果仍可操作。產品版本 0.28.0。
- 後續里程碑：M26 根目錄範圍合併（預定 0.29.0），見 SPEC §37／D045。現行重疊拒絕限制尚未修改；先完成 M25 公司驗收，再實作 M26。
- 本機實作重點：搜尋工作階段只保留條件、文件識別與命中 metadata；每層只核對目前候選，翻頁不重核對，顯示時才載入片段。追加條件檢查未顯示頁與片段外正文。縮小集合保留最初相對排序。`--page`／`--limit` 與非互動輸出沿用 0.27.0 行為。
- 下一步請使用者在公司 Windows 執行「初搜 → 多次追加 → 翻頁 → back → reset」。0.27.0 人工驗收成功不代表 0.28.0 已通過。未另提供新版完整 Windows 自動測試結果；既有 PPTX 缺部件問題不因本版而結案。
- 2026-09-22 Linux 開發沙盒 `npm test`：167 項中 164 通過、1 失敗、2 略過。新增 12 項 M25 回歸全部通過。失敗項為既有 M7 開啟在非 darwin／win32 回報 `ACTION_PLATFORM_UNSUPPORTED`，不是本次搜尋行為回歸。略過項為 M5 無法讀取目錄（此環境）及 Windows cmd 專屬測試。不得把此沙盒結果宣稱為 macOS 全套通過。交付包逐檔核對 188 個檔案：`LocalDocSearch-M25-0.28.0.zip`，SHA-256 `ee52ecfa2edc39e2f387a38db11700b812285a8524b59043686e5de06542e5d5`。
- 使用者於 2026-09-22 回報 0.27.0 公司 Windows 人工驗收成功。該紀錄仍成立，但不覆蓋 0.28.0。
