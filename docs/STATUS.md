...
最後更新：2026-09-22（0.27.0 人工驗收成功；M25 結果內搜尋規劃）

## 目前狀態

- 目前進行中里程碑：M25 結果內搜尋（預定 0.28.0），依使用者要求加入正式規劃，見 SPEC §36／D044；尚未實作，產品版本仍為 0.27.0。
- 後續里程碑：M26 根目錄範圍合併（預定 0.29.0），見 SPEC §37／D045。使用者要求將上層 index 的修正架構加入 SPEC；規劃原子轉移既有子根文件、保留排除作用域、子樹同步與相關命令相容。本次僅文件更新，現行重疊限制尚未修改；先 M25、再 M26。
- 使用者於 2026-09-22 回報新版 0.27.0 公司 Windows 人工驗收成功。未另提供新版完整自動測試結果，不將人工驗收擴寫為全套自動測試通過，也不推定既有 PPTX 問題已解決。
- 下一步依規劃實作搜尋工作階段，再接 CLI `/ 關鍵字`、back／reset、條件鏈及分頁，最後補齊跨頁／片段外命中、回退與索引變更回歸。本次僅更新規劃與驗收紀錄，未修改程式或重新發版。
- 搜尋現在回報完整命中總數、頁碼及本頁範圍。TTY 預設每頁 20 筆並接受 n／p／q；非互動使用 `--page`／`--page-size` 並顯示下一頁提示。`--limit` 保留為互斥的單次輸出。命中排序只建立一次，每頁才回讀片段；翻頁期間若 SQLite `data_version` 改變則回報 `SEARCH_INDEX_CHANGED`。
