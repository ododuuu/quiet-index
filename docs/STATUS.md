...
最後更新：2026-09-22（0.31.0 背景自動更新規格已完成，待實作）

## 目前狀態

- 目前進行中版本：**0.31.0 背景自動更新**。SPEC §39 與 D047 已完成，程式尚未實作；下一步由 Grok 依規格實作並更新測試。從 0.31.0 起以版本號作唯一里程碑名稱，不再新增 M 編號。
- 0.31.0 目標：提供 `autoupdate start|status|stop`、可驗證的本機單例背景程序、事件佇列與精確檔案／子樹更新、6 小時完整增量校正、動態 roots、降級復原、安全停止及有界日誌。本版不做開機／登入自啟，不安裝 Windows Service，0.30.0 索引可直接使用。
- 編碼政策已鎖定：明確 BOM／XML 宣告優先；無訊號時整份嚴格 UTF-8，失敗才整份嚴格 Big5。精度優先，不加統計猜測或要求使用者指定目錄編碼。
- 0.30.0 原始碼、Big5 與索引觀測 **已完成實作**。產品 package 仍為 0.30.0；公司 Windows 請以普通 `index` 升級既有 0.29.1 索引後核對中文命中、行號、XML 錯誤碼、無變更重跑、搜尋延遲與新容量。
- 本次只更新 0.31.0 規格、狀態、決策、路線圖與交接，未修改程式也未執行產品測試；不可將規格完成寫成 0.31.0 已實作。
