...
最後更新：2026-09-22（0.29.0 M26 根目錄範圍合併已實作；公司 Windows 擴大根目錄待驗收）

## 目前狀態

- 目前進行中里程碑：M26 根目錄範圍合併（0.29.0）已依 SPEC §37／D045 實作。`index` 上層路徑以路徑元件判斷涵蓋，短交易轉移 `document_roots` 並保留排除作用域；不得呼叫會刪文件的 removeRoot。已登錄上層時 index 子樹只同步該範圍，成功不更新上層最後完整同步時間。`--root`、remove、rebuild、watch、open／reveal 協同理解合併後歸屬。產品版本 0.29.0。
- 使用者於 2026-09-22 回報 0.28.0 驗測成功。該紀錄不擴張為公司 Windows 全套自動測試已通過，也不覆蓋 0.29.0。
- 下一步請使用者在公司 Windows 以既有 `D:\備份` 索引執行 `index D:\`，確認原文件代碼與搜尋可用、其他可讀位置新增、無權限位置有明確診斷且不丟失舊資料、roots 無重複、重跑為增量。
- 2026-09-22 Linux 開發沙盒 `npm test`：181 項中 178 通過、1 失敗、2 略過。新增 14 項 M26 回歸全部通過。失敗項為既有 M7 開啟在非 darwin／win32 回報 `ACTION_PLATFORM_UNSUPPORTED`，不是本次根目錄合併回歸。略過項為 M5 無法讀取目錄（此環境）及 Windows cmd 專屬測試。不得把此沙盒結果宣稱為 macOS 全套通過或 Windows 驗收。交付包逐檔核對 193 個檔案：`LocalDocSearch-M26-0.29.0.zip`，SHA-256 `006f31d6ae6a363e08e8e5f8dfd50f929065e28a604c314971078d54b6884dde`。
