...
最後更新：2026-09-22（0.29.1 修正 Windows 磁碟根目錄輸入；M26 公司擴大根目錄待驗收）

## 目前狀態

- 目前進行中里程碑：M26 根目錄範圍合併。0.29.1 修正 Windows 將 `D:\`／全形 `＼` 判成路徑不完整：輸入層把僅磁碟代號、`D:/` 與全形分隔符視為 `D:\`。公司請改試 `index D:/`（加引號也不要讓路徑以反斜線結尾）。
- 使用者於 2026-09-22 回報 0.28.0 驗測成功，並回報 0.29.0 的 `D:\` 輸入失敗；該失敗已在 0.29.1 修正，尚未再次驗收。
