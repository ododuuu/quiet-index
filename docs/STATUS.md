...
最後更新：2026-09-22（M27／0.30.0 已實作）

## 目前狀態

- 目前進行中里程碑：M27／0.30.0 原始碼、Big5 與索引觀測 **已完成實作**。產品 package 為 0.30.0。公司 Windows 請以普通 `index` 升級既有 0.29.1 索引後核對中文命中、行號、XML 錯誤碼、無變更重跑、搜尋延遲與新容量；只回報統計與錯誤碼，不需上傳公司內容。
- 本版交付：`.java`／`.sql`／`.js` 原文（`.class` 僅檔名）、共用嚴格解碼（BOM／XML 宣告 → UTF-8 → Big5）、逐文件 `parse_version=1` 讓舊 TXT／MD／AsciiDoc／XML 與新格式 unsupported 一次 index 即升級、TTY／非 TTY 節流百分比進度、未變更計數修正、結尾診斷彙總、`status` 容量與 `--issues`／`--types`。背景自動更新管理仍為獨立待辦。
