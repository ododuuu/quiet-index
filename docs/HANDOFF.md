...
## 後續規劃：M26 根目錄範圍合併（2026-09-22）

使用者要求將加入上層根目錄的修改架構寫入 SPEC，見 §37／D045。規劃以短交易把既有子根歸屬轉到上層，保留文件 ID／payload／排除作用域，再增量掃描；已登錄上層時 index 子樹不另建根，刪除校正限子樹。搜尋 root 篩選、watch、open／reveal、remove／rebuild 必須協同調整。排程先 M25 結果內搜尋，再 M26（預定 0.29.0）；本次未改程式，0.27.0 仍有重疊拒絕限制。

## 最新規劃：M25 結果內搜尋（2026-09-22）

使用者已回報 0.27.0 公司 Windows 人工驗收成功，並確認將結果內搜尋加入規劃。SPEC §36／D044 定義 `/ 關鍵字` 追加文件級 AND、back／reset、完整條件鏈及總數。條件須核對目前全部命中文件的完整已索引內容，不能只查當頁或片段。下一里程碑預定 0.28.0；本次只更新文件，程式仍為 0.27.0、尚未實作結果內搜尋。Windows 完整自動測試未另回報；既有 PPTX 缺部件問題不因人工驗收而自動結案。
