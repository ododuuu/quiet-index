...
依 SPEC §36／D044 實作互動結果內搜尋。`search` 工作階段以 `/ 關鍵字` 對目前完整命中集合追加文件級 AND，核對檔名、標題與全部已索引正文（含未顯示頁與片段外內容）；back／reset 可逆，單頁與零結果仍可操作。縮小後保留最初相對排序，片段展示最新一層條件。非互動／`--page`／`--limit` 行為不變。索引變更回報 `SEARCH_INDEX_CHANGED`。程式版本 0.28.0；公司 Windows 實機流程待驗收。下一里程碑仍為 M26（§37／D045）。

## 後續規劃：M26 根目錄範圍合併（2026-09-22）

使用者要求將加入上層根目錄的修改架構寫入 SPEC，見 §37／D045。規劃以短交易把既有子根歸屬轉到上層，保留文件 ID／payload／排除作用域，再增量掃描；已登錄上層時 index 子樹不另建根，刪除校正限子樹。搜尋 root 篩選、watch、open／reveal、remove／rebuild 必須協同調整。排程先完成 M25 公司驗收，再實作 M26（預定 0.29.0）；0.28.0 仍有重疊拒絕限制。
