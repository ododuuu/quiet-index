...
- 日期：2026-09-22。使用者確認下一版加入 java／sql／js、Big5、簡潔進度及容量觀測，class 不需正文支援；規格見 SPEC §38。Codex 負責規格與 Git 提交，Grok 負責實作。
...
- 1113.18 MiB 已獲使用者接受，先觀測新格式影響；不自動 VACUUM，不改系統目錄排除範圍。背景自動更新管理及具體 PDF／PPTX 修復另列待辦。0.29.1 人工成功不等於同步完整或所有文件解析成功。
