...
- 日期：2026-09-22。使用者回報 0.27.0 人工驗收成功後，確認需要結果內搜尋，並要求加入規劃；本決策尚未實作。
...
- 日期：2026-09-22。使用者要求把 `D:\備份` 擴大到 `D:\` 的支援架構加入 SPEC。§37 取代父子根一律拒絕的預定行為，列為 M26，尚未實作。
- 保留單一文件歸屬，新增根目錄操作計畫及短交易轉移，不以刪除原根再重新索引實作合併。文件 ID、payload 與既有搜尋代碼均保留；長掃描在合併提交後進行，中斷仍有可讀的舊成果。
- 保留原子根的排除規則作用域，避免合併讓原先排除的資料被意外納入；上層規則與適用子根規則共同約束。已涵蓋子樹可單獨增量同步，但不得宣稱上層已完整同步。
- 同步、搜尋篩選、監看與根移除需共同理解合併後的歸屬；不能只移除重疊檢查。詳細相容及故障測試依 SPEC §37。
