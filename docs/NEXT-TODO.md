...
更新：2026-09-23。0.36.0 本機實作已完成；下列公司複驗仍開著。已完成搜尋核心、格式解析、背景更新、TUI、MCP／MCP App、本機拖曳工作台，以及索引沿用／TUI 退出修正。

## 當前優先：公司複驗 0.36.0

使用者回報的 0.35.0 問題裡，本機已重現並修好 TUI Ctrl+C／EOF，也量到大型無關 mapping 的替換熱點。公司索引慢速仍待同一台電腦的匿名 profile。不要把下面未勾項當成程式還沒寫。

- [x] 沿用既有資料庫，區分一次性文字升級／新格式／錯誤重試／未變更，已完成成功文件重跑零解析。
- [x] 階段計時與匿名本機 profile、大型舊庫替換基線、按實測修正 SQL 熱點並核對外鍵。
- [x] TUI 退出生命週期與真實 PTY 回歸，help 容錯／補全、固定輸入列、可翻頁預覽與中文寬度。
- [ ] 公司 Windows 對現有索引做普通 index、`--profile`、中斷接續、無變更重跑與 TUI。不得以本機合成基準代替。
